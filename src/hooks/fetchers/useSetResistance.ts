...
import { IRTable, table as initTable, PKMN_JSON, TYPE_JSON, TYPE_LIST } from "utils/Interfaces";
  ...
  useEffect(() => {
    if (!typeLoading && !pkmnLoading) {
      ...
      pkmnData?.forEach((pokemon, index) => {
        ...
        let { weaknesses: weak_to, resistances: resists, immunities: immune_to } = setRelations(types, typeData!);
